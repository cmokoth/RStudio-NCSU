...
The data for this project come from MIT's Persuasion for Good dataset </link here> [the name and hyperlink].
I am loading a number of packages here, marked eack for the different sections they're used in. Some are essential [`r tidyverse`] while others are nice to have [`r skimr`] 
...
I pulled the raw data from the repository, just `full_d_raw` and `full_i_raw`, which are the full dialogue and information about each participant. The raw datasets the basics. here's where I got it/how I did it process 
...
To create a ranked set, we use `ConvoId` as a 
```{r tf_idf}
