...
This project is an exploration of text processing using methods from term frequency to unsupervised clustering. The data for this project come from MIT's Persuasion for Good dataset [@wang2019persuasion].
...
The data are from the [MIT gitlab page](https://gitlab.com/ucdavisnlp/persuasionforgood/tree/master/data). I pulled the raw data directly from the repository, just `full_d_raw` and `full_i_raw`, which are the full dialogue and information tables about each participant (respectively). 
...
# full_i for the full participant table, not annotated
...
`tidytext::get_stopwords()` is an important part of this process. It identifies words that do not usually contain meaningful information so that the tens of thousands of common little words won't interfere with our analysis [like 'and' or 'to']. Nesting the function within `anti_join()` removes these observations from the dataset.
...
fulltidy %>% count(Word, sort = TRUE) %>% 
  ...
  geom_col() +
  coord_flip() +
  labs(x = NULL, title = "Most Common Words in Dialogue", subtitle = "after removing stopwords")
```
...
fulltidy %>% count(Word,Role, sort = TRUE) %>% 
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Top Words by Role", y = "# of times word used", x = NULL,fill = NULL,subtitle = "Only the persuaders said payment (persuaders said every other word too)")
```
...
fulltidy %>% 
  ...
  slice_max(n,n = 20) %>% 
  complete(Role,Word,fill = list(n=0)) %>% 
  ggplot(aes(Word,n)) +
  geom_col(aes(fill = Role),show.legend = FALSE) +
#  geom_text(aes(label = Word), check_overlap = TRUE, vjust = 1.5) +
  facet_wrap(~Role, scales = "free") +
  labs(x = NULL,y = "# of times word used",title = "Most Common Words by Role", subtitle = "Persuaders talked waaaay more often...about the same things as the persuadees") +
  coord_flip()
```

Role also seems to have an impact of how *much* each participant says. Looking at the y axis, we see that the scales are wildly different because of how much more the persuaders speak. Clearly, the persuaders are driving the conversation. This chart shows the most common words by Role. One would think, 'Ah this is it! We know what the most common words by Role are!' Well, not exactly. Alas, this is only the beginning of text analysis. This chart is still important though! 
...
annset %>% 
  distinct(ConvoId, Role, IntDonation) %>% 
  filter(IntDonation < 200,
         Role == 1) %>% 
  ggplot() +
  geom_histogram(aes(IntDonation)) +
...
Not a lot of people planned to donate! Our persuaders may not have been very persuasive. There were a few outliers not represented on this graph, but first let's see how the money actually turned out.
...
The participants self identified into a group.
...
wordcount %>% 
  ...
  labs(title = 'Word Frequency per Conversation', x = "proportion of word occurence", y = '# of words',subtitle = 'Many words are used infrequently and a few words are used very frequently') 
```
...
<!-- concptual explanation | reference -->
$$
...
where tf is term frequency, df is document frequency, and N is number of documents. Term frequency is simple frequency within the document, and inverse document frequency is the logarithm of number of documents over the number of documents containing the term.
...
role_words %>% 
  ...
  labs(title = "Highest tf-idf words by Role", y = NULL, subtitle = " 'u sir' is not a very good description of our dataset")
```
...
tf-idf, or **term frequency-inverse document frequency** requires adequate word counts, and in our case was an ineffective approach, as the relationship between word counts and document sizes (individual conversations) had too few words in it. With how many observations each word had, there are not high enough  word counts to distinguish which words are valuable and which words are not. Some words had such low counts (typos, speaking styles), tf-idf was useless in identifying their importance as typos or important words.
...
Weighted Log Odds is a technique comparing the frequency of the words in the text to the frequency of the words in each grouping. This technique differs from term frequency-inverse document frequency, because tf-idf uses the frequency of the words and compares which words are different across documents, using a relative search to estimate importance. This log-odds approach is different, using logarithm of the odds and empirical Bayes [@baseball; @efrom2013] to weight variable across groups. The main difference is that rather than measuring simple frequency per Word per group, we measure the logarithm of the odds and weigth it using empirical Bayes. What are we left with? 
...
agree_slopes <- agree_freq %>%
  ...
  # running a generalized linear model on each value-count combination using agreeable score 
...
conscience_slopes %>%
  ...
  facet_wrap(~Word) +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = "The Big Five: Conscientiousness",x = "Conscientiousness score", y = "Word frequency", subtitle = "Conscientious people are less likely to say donate and know but more likely to say yes")
```
...
Important note: Because the 9th p.value ["0"] is 1, `slice_min()` fails here, as the remaining effect sizes are also 1. Here we just have to use the smallest values of `arrange()`As we can see, extroverts are less likely to say many of these words, including 'bad' and 'point' but much more likely to say 'time' and 'weekend'. 
<!-- say more words -->
Now for neuroticsm.
...
```{r topic_model00, eval = FALSE, include = FALSE}
...
```{r topic_model01, eval = FALSE, include = FALSE}
library(stm)
...
```{r topic_model02, eval = FALSE, include = FALSE}
plan(multiprocess)
...
SC: Topic Quality
K =? 60-70
```{r topic_model03, eval = FALSE, include = FALSE}
k_result %>%
...
```{r topic_model04, eval = FALSE, include = FALSE}
