...
more words to describe interesting things or essential information (what's in here?) err on the side of text rather than comments
...
Here is where the tidying happens! So tidy text data is *one observation per row*, and in this text project, that means breaking up the text container `Dialogue` and having one word per row. I used `tidytext` to automatically parse the text into single rows.

get_stopwords() was an important part of this process. It removes words that do not usually contain meaningful information so that the tens of thousands of random little words won't interfere with our analysis.
...
full_i_raw %>% 
  ...
  geom_col(show.legend = FALSE) +
  facet_wrap(~edu) +
  labs(title = "Who's Talking?", x = 'political ideology', y = '# of participants', subtitle = "Most people have don't have a college degree, but similar political ideas")
```
