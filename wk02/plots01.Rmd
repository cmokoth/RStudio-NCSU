...
fulltidy %>% count(Word, sort = TRUE) %>% 
  ...
  labs(x = NULL)
```
Our data are held as full exchanges in a dialogue, where each exchange can comprise of multiple sentences. The dialogue consists of typed exchanges between participants.
Using 'tidy' data, we can see which words were used the most during the dialogues across the whole set, including a number of words such as 'can' and 'like', words that could be filtered our due to their lack of inherent meeting. The words 'children','save',and 'donate' were used the most out of any words, most likely because the charity is called 'Save the Children' and the participants are tasked with convincing people to donate.
...
#  geom_text(aes(label = Word), check_overlap = TRUE, vjust = 1.5) +
  ...
  coord_flip()
```
The words each role said most were
...
```
The top words tell us some basic information about our data. We see the name of the charity 'Save [the] Children' and we see the task objective 'donate'. We also see words related to money, 'donation','payment','amount'. There are also affirming words, 'yes','great','good' and other adjectives that would help convince people.
Text analysis is based around frequency, how often the words occur. And there is something meaningful to learn from the different methods of exploring frequency we go over in this analysis.
...
```
Here we have our distributions of personality traits. This study collected data for three major personality scores: the big five, the schwartz and the moral foundations. The big five show us that our sample has very agreeable, extroverted, and concientious participants. They also skew towards open. 
...
```
Here we see high levels of fariness and care, indicating a compassionate bunch.
...
full_i_raw %>% 
  ...
  geom_histogram(bins = 6, show.legend = FALSE)
```
The Schartz protrait shows benevolence, self-direction, and universalism
